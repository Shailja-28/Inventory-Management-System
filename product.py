...
class productClass:
    def __init__(self,root):
        ...
        #=====================Title=========================
        title=Label(product_Frame,text="Manage Product Details",font=("goudy old style",18,"bold"),bg="#0f4d7d",fg="white").pack(side=TOP,fill=X)
        #===========Column1===================
        lbl_category=Label(product_Frame,text="Category",font=("goudy old style",18,"bold"),bg="white").place(x=30,y=60)
        lbl_supplier=Label(product_Frame,text="Supplier",font=("goudy old style",18,"bold"),bg="white").place(x=30,y=110)
        lbl_name=Label(product_Frame,text="Name",font=("goudy old style",18,"bold"),bg="white").place(x=30,y=160)
        ...
        p_frame.place(x=480,y=90,width=600,height=370)
    ...
    def add(self):
        ...
        try:
            if self.var_cat.get()=="Select" or self.var_cat.get()=="Empty" or self.var_name.get()=="" or self.var_sup.get()=="Select":
                ...
                row=cur.fetchone()
            if row!=None:
                messagebox.showerror("Error","This product already exist, try another",parent=self.root)
            else:
    ...
    def search(self):
        ...
        try:
            if self.var_searchby.get()=="Select":
                ...
                cur.execute("select * from product where "+str(self.var_searchby.get())+" LIKE '%"+str(self.var_searchtxt.get())+"%'")
                rows=cur.fetchall()
                if len(rows)!=0:
                    self.product_table.delete(*self.product_table.get_children())
                    for row in rows:
